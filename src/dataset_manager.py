...
class DatasetManager:
    ...
    def load_file_from_github(
        self, repo_name: str, branch: str, file_path: str
    ) -> List[Dict[str, Any]]:
        import requests
        import json
        ...
        if response.status_code != 200:
            raise Exception(
                f"Erro ao baixar o arquivo do repositório {repo_name}: {response.status_code}"
            )
    ...
    def load_oab_bench_reference_answers(self) -> List[Dict[str, Any]]:
        ...
        """
        questions = self.load_file_from_github(
            self.OAB_BENCH_REPO_NAME,
            self.OAB_BENCH_COMMIT,
